...
const logLinkedInToIntegration = async ({ lead_id, activity }) => {
  try {
    ...
    if (
      ...
      lead?.User?.Company?.Company_Setting?.activity_to_log?.LINKEDIN
        ?.enabled ||
      true
    ) {
      let access_token = '',
        ...
        case CRM_INTEGRATIONS.PIPEDRIVE:
          ...
          const [pipedriveData, pipedriveDataError] = await createNoteActivity({
            ...
            integration_data: {
              access_token,
              instance_url,
              content: `<B>${activity.name}</B><br>${activity.status}`,
        ...
        case CRM_INTEGRATIONS.HUBSPOT:
          ...
          const [hubspotData, hubspotDataError] = await createNoteActivity({
            ...
            integration_data: {
              ...
              hs_note_body: `<h4>${activity.name}</h4>${activity.status}`,
        ...
        case HIRING_INTEGRATIONS.BULLHORN:
          ...
          switch (lead.integration_type) {
            case LEAD_INTEGRATION_TYPES.BULLHORN_CONTACT:
              bullhornContent = {
                action: 'Other',
                comments: `<b>Title :</b> ${activity.name} <br>
...
<b>Content :</b> ${activity.status}`,
        ...
        case CRM_INTEGRATIONS.SELLSY:
          ...
          if (access_token === null) {
            logger.error('Please sign in with Sellsy: ', errForAccessToken);
            return [null, errForAccessToken];
          }
          ...
          const [sellsyData, sellsyDataError] = await createNoteActivity({
            ...
            integration_data: {
              access_token,
              created: new Date(),
              description: `${activity.name ?? ''} ${activity.status ?? ''}`,
              related: [
                {
                  id: parseInt(lead.integration_id),
    ...
    logger.error(`Error while logging linkedIn activity: `, error);
    return [null, error];
  }
};
module.exports = logLinkedInToIntegration;
