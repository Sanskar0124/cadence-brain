...
const removeContactDuplicate = async () => {
  try {
    logger.info('Removing contact duplicate.');
    let [leads, errForLeads] =
      await LeadRepository.getDuplicateBySalesforceContactIdCount();
    if (errForLeads);
...
module.exports = removeContactDuplicate;
