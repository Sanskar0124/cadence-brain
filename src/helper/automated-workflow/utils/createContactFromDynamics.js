...
const createContactFromDynamics = async ({ lead, company_id }, t) => {
  try {
    logger.info('Creating contact from dynamics...');
    let account = null,
      errForAccount,
      errForAccountFetch;
    // * Handle company
          ...
          if (errForAccount.includes('must be unique')) {
            ...
            if (errForAccountFetch) {
              ...
              return [
                null,
                {
                  error: `Error while fetching account: ${errForAccountFetch}`,
    ...
    if (errForLead) {
      if (errForLead.includes('must be unique')) {
        ...
        if (errFetchingExistingLead)
          return [
            null,
            `Lead exists but unable to fetch : ${errFetchingExistingLead}`,
          ];
        if (!existingLead) return [null, `Lead exists but unable to fetch`];
...
module.exports = createContactFromDynamics;
