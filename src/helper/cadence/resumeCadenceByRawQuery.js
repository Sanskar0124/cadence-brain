...
const resumeCadenceByRawQuery = async ({
  cadence_id,
  create_from_node_id,
  resume_from_node_id,
  startTimeMap = {},
  ...
  try {
    ...
    console.log('in resumeCadenceByRawQuery');
    console.log(
      JSON.stringify(
        {
          cadence_id,
...
${startTimeConditions}
    ...
    // TODO: remove once feature is stable
    console.log(metadata, startTimeCase);
...
inner join lead_to_cadence ltc on t.lead_id = ltc.lead_id and t.cadence_id = ltc.cadence_id 
inner join settings s on s.user_id = t.user_id
...
where 
 t.node_id = :resume_from_node_id and t.completed = 0 and t.is_skipped = 0
),
...
-- select * from all_tasks
    ...
    // TODO: remove once feature is stable
    console.log('resumeCadenceByRawQuery');
    console.log({
      cadence_id,
      create_from_node_id,
      resume_from_node_id,
    ...
    logger.error(`Error while resuming cadence using raw query: `, err);
    return [null, err.message];
  }
};

module.exports = resumeCadenceByRawQuery;
