...
const ShareCadenceToGroups = async (
  { cadence_id, user, object, is_workflow, toShareSubDepartment, ogCadence },
  t
) => {
  try {
    let createCadences = [];
    let cadenceName = object.name;
    for (let i = 0; i < toShareSubDepartment.length; i++) {
      if (i != 0) object.name = `${cadenceName} (${i})`;
      object.sd_id = toShareSubDepartment[i].sd_id;
      ...
      const [access, errForAccess] = checkCadenceActionAccess({
        ...
        data: { ogCadence, toShareSubDepartment: toShareSubDepartment[i] },
      });
      if (errForAccess) {
        return [
          null,
...
module.exports = ShareCadenceToGroups;
