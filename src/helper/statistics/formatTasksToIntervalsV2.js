const formatTasksToIntervalsV2 = (tasks, timeZone = null, heatMap) => {
  // changine complete_time of current task to the user specific timezone
  for (let task of tasks) {
    const timeZonedDate = new Date(
      new Date(task.complete_time).toLocaleString('en-US', {
    ...
    heatMap[dayOfWeek][hourOfDay] =
      parseInt(heatMap[dayOfWeek][hourOfDay], 10) + 1;
  }

  return heatMap;
};

module.exports = formatTasksToIntervalsV2;
