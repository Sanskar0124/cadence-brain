...
const checkIfDailyLimitReachedForEmail = async (
  salesPerson,
  userTask,
  ...
  try {
    ...
    if (userTask && automatedTaskSettings) {
      if (
        userTask.automated_mails_sent_per_day >=
        automatedTaskSettings.max_emails_per_day
      ) {
        logger.info(
    ...
    logger.error(`Error while checking if daily limit reached: `, err);
    return [null, err.message];
  }
};

module.exports = checkIfDailyLimitReachedForEmail;
